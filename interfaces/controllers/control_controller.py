...
async def create_control_endpoint(control: ControlCreate, db: AsyncSession = Depends(get_async_session)):
...
async def read_control_endpoint(control_id: int, db: AsyncSession = Depends(get_async_session)):
    repository = ControlRepository(db)
...
async def read_all_controls_endpoint(db: AsyncSession = Depends(get_async_session)):
    repository = ControlRepository(db)
...
async def update_control_endpoint(control_id: int, control: ControlCreate, db: AsyncSession = Depends(get_async_session)):
    repository = ControlRepository(db)
...
async def delete_control_endpoint(control_id: int, db: AsyncSession = Depends(get_async_session)):
    repository = ControlRepository(db)
