...
async def create_risk_category_endpoint(risk_category: RiskCategoryCreate, db: AsyncSession = Depends(get_db)):
...
async def read_risk_category_endpoint(risk_category_id: int, db: AsyncSession = Depends(get_db)):
    repository = RiskCategoryRepository(db)
    use_case = ManageRiskCategoriesUseCase(repository)
...
async def read_all_risk_categories_endpoint(db: AsyncSession = Depends(get_db)):
    repository = RiskCategoryRepository(db)
    use_case = ManageRiskCategoriesUseCase(repository)
...
async def delete_risk_category_endpoint(risk_category_id: int, db: AsyncSession = Depends(get_db)):
    repository = RiskCategoryRepository(db)
    use_case = ManageRiskCategoriesUseCase(repository)
    try:
