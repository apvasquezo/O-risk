...
async def create_risk_category(risk_category: RiskCategoryCreate, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return RiskCategoryResponse(id=created_risk_category.id, description=created_risk_category.description)
...
async def read_risk_category(risk_category_id: int, db: AsyncSession = Depends(get_db)):
    ...
    return RiskCategoryResponse(id=category.id, description=category.description)
...
async def read_all_risk_categories(db: AsyncSession = Depends(get_db)):
    ...
    return [RiskCategoryResponse(id=category.id, description=category.description) for category in categories]
...
async def update_risk_category(risk_category_id: int, risk_category: RiskCategoryCreate, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return RiskCategoryResponse(id=updated_category.id, description=updated_category.description)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
