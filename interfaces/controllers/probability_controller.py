...
async def create_probability_endpoint(probability: ProbabilityCreate, db: AsyncSession = Depends(get_async_session), _: None = Depends(role_required("super"))):
...
async def read_probability_endpoint(probability_id: int, db: AsyncSession = Depends(get_async_session), _: None = Depends(role_required("super"))):
    repository = ProbabilityRepository(db)
...
async def update_probability_endpoint(probability_id: int, probability: ProbabilityCreate, db: AsyncSession = Depends(get_async_session),_: None = Depends(role_required("super")) ):
    repository = ProbabilityRepository(db)
...
async def delete_probability_endpoint(probability_id: int, db: AsyncSession = Depends(get_async_session), _: None = Depends(role_required("super"))):
    repository = ProbabilityRepository(db)
