...
class MacroprocessRepository:
    ...
    async def create_macroprocess(self, macroprocess: Macroprocess) -> Macroprocess:
        ...
        ).returning(ORMMacroprocess.id, ORMMacroprocess.description)
        try:
            result = await self.session.execute(stmt)
            await self.session.commit()
            row = result.fetchone()
            if row:
                return Macroprocess(id=row.id, description=row.description)
    ...
    async def get_macroprocess(self, macroprocess_id: int) -> Optional[Macroprocess]:
        stmt = select(ORMMacroprocess).where(ORMMacroprocess.id == macroprocess_id)
        result = await self.session.execute(stmt)
        orm_macroprocess = result.scalar_one_or_none()
        if orm_macroprocess:
            return Macroprocess(id=orm_macroprocess.id, description=orm_macroprocess.description)
        return None
    ...
    async def get_all_macroprocesses(self) -> List[Macroprocess]:
        ...
        return [Macroprocess(id=m.id, description=m.description) for m in orm_macroprocesses]

    async def update_macroprocess(self, macroprocess_id: int, macroprocess: Macroprocess) -> Optional[Macroprocess]:
        stmt = update(ORMMacroprocess).where(ORMMacroprocess.id == macroprocess_id).values(
            description=macroprocess.description
        ).returning(ORMMacroprocess.id, ORMMacroprocess.description)
        result = await self.session.execute(stmt)
        await self.session.commit()
